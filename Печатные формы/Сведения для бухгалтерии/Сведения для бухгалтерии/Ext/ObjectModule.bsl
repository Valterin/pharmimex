...
Функция СформироватьПечатнуюФорму(СсылкаНаДокумент, ОбъектыПечати)Экспорт
	...
	// +Дмитрий@, 2024-10-24 07:01:25 HR-005. Сведения для бухгалтерии 
	...
	|	ЕСТЬNULL(ДокументыФизическихЛицСрезПоследних.КодПодразделения, 0) КАК КодПодразделения,
	...
	|					ИЛИ СрокДействия = ДАТАВРЕМЯ(1, 1, 1, 0, 0, 0))) КАК ДокументыФизическихЛицСрезПоследних
	...
	Пока Выборка.Следующий() Цикл 		
		...
		ТекстКемВыдан = СтрШаблон("%1, дата выдачи: %2, код подразделения: %3",
		СокрЛ(Выборка.КемВыдан),
		Формат(Выборка.ДатаВыдачи, "ДФ=dd.MM.yyyy"),
		СокрЛ(Выборка.КодПодразделения));
		
		ОбластьДанные.Параметры.ДокументКемВыдан     = ТекстКемВыдан;
