...
Функция СформироватьПечатнуюФорму(СсылкаНаДокумент, ОбъектыПечати)Экспорт
	...
	Работник.Параметры.РаботаТрудовойДоговорНомер                  = ПолучитьНомерНаПечать(ТЗ[0].Сотрудник);
	...
	Если НЕ  ТЗ[0].СтатьяТКРФ = Неопределено Тогда
		Работник.Параметры.РаботаСтатьяТКРФНаПечать =       ТЗ[0].СтатьяТКРФ.Наименование + ", " + ТЗ[0].СтатьяТКРФ.ТекстОснования;
	КонецЕсли;
