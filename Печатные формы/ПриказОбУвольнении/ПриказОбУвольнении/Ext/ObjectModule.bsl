...
Функция СформироватьПечатнуюФорму(СсылкаНаДокумент, ОбъектыПечати)Экспорт
	...
	Если ЗначениеЗаполнено(ТЗ[0].СсылкаНаОбъектНомерПриказа) Тогда
		Шапка.Параметры.СсылкаНаОбъектНомерНаПечать = ТЗ[0].СсылкаНаОбъектНомерПриказа; 
	Иначе 
		
		Если ЗначениеЗаполнено(ТЗ[0].СсылкаНаОбъектНомер) Тогда
			Шапка.Параметры.СсылкаНаОбъектНомерНаПечать = УбратьЛидирующиеНули(СокрЛП(ТЗ[0].СсылкаНаОбъектНомер));
		КонецЕсли;  
	КонецЕсли;
	...
	Работник.Параметры.РаботаТрудовойДоговорДатаОформленияНаПечать = Формат(ТЗ[0].ДатаДоговораКонтракта,"ДФ=dd.MM.yyyy; ДЛФ=DD");
	Работник.Параметры.РаботаТрудовойДоговорНомер                  = ТЗ[0].НомерДоговораКонтракта;
	Работник.Параметры.РаботаДатаУвольненияНаПечать = Формат(ТЗ[0].ДатаУвольнения,"ДФ=dd.MM.yyyy; ДЛФ=DD");
	...
	Если НЕ  ТЗ[0].СтатьяТКРФ = Неопределено Тогда
		Работник.Параметры.РаботаСтатьяТКРФНаПечать =  ТЗ[0].СтатьяТКРФ.ПричинаУвольненияПФР.Наименование + " " + ТЗ[0].СтатьяТКРФ.ДокументОснование + ", " + ТЗ[0].СтатьяТКРФ.ТекстОснования;
	КонецЕсли;
	...
	//Подвал 
	ПодвалРуководитель = МакетОбработки.ПолучитьОбласть("ПодвалРуководитель"); 
	ПодвалРуководитель.Параметры.СсылкаНаОбъектДолжностьРуководителя = ТЗ[0].СсылкаНаОбъектДолжностьРуководителя ;
	ПодвалРуководитель.Параметры.СсылкаНаОбъектРуководительРасшифровкаПодписи =ТЗ[0].РуководительРасшифровкаПодписи ; 
	//Подписант1
	ПодвалРуководитель.Параметры.ПодписантКадровыхДокументов = УправлениеСвойствами.ЗначениеСвойства(СписокПриказов[0].Организация, ПодписантКадровыхДокументов1);
	ТабличныйДокумент.Вывести(ПодвалРуководитель);
	
	ПодвалПодписант = МакетОбработки.ПолучитьОбласть("ПодвалПодписант"); 
	Подписант2 = УправлениеСвойствами.ЗначениеСвойства(ТЗ[0].Подразделение, ПодписантКадровыхДокументов2);
	Если ЗначениеЗаполнено(Подписант2) Тогда  
		//Подписант 2
		ПодвалПодписант.Параметры.ПодписантКадровыхДокументов = Подписант2;  
		ТабличныйДокумент.Вывести(ПодвалПодписант);
	КонецЕсли;
	//Подписант3
	ПодвалПодписант.Параметры.ПодписантКадровыхДокументов = УправлениеСвойствами.ЗначениеСвойства(СписокПриказов[0].Организация, ПодписантКадровыхДокументов3); 
	ТабличныйДокумент.Вывести(ПодвалПодписант);
	
	////Подвал.Параметры.СсылкаНаОбъектДатаОзнакомленияРаботника =  ;
	//ТабличныйДокумент.Вывести(ПодвалРуководитель); 
	
	ПодвалРаботник = МакетОбработки.ПолучитьОбласть("ПодвалРаботник");	
	ТабличныйДокумент.Вывести(ПодвалРаботник); 
...
Функция ПолучитьНомерНаПечать(Сотрудник)
...
Функция УбратьЛидирующиеНули(Строка)
	...
	Если Массив.Количество() = 3 Тогда 
		...
		Возврат Строка(Число(Массив.Получить(1))) + "-" + Массив.Получить(2); 
